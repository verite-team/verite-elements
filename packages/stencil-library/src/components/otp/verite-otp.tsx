...
export class VeriteOtp {
  ...
  private focusActive() {
    setTimeout(() => {
      const activeTextbox = this.el.shadowRoot?.querySelector(`vf-textbox[name="code-${this.activeIndex}"]`)
      if (activeTextbox) {
  ...
  //     setTimeout(() => {
  //       const nextTextbox = this.el.shadowRoot?.querySelector(`vf-textbox[name="code-${index + 1}"]`)
  //       if (nextTextbox) {
  ...
  render() {
    return (
      <Host onKeyDown={this.handleKeyDown}>
        <div class="verite-otp">
          <vf-card-content>
            <form onSubmit={this.handleSubmit}>
              <vf-flex direction="row" halign="center" gap={2}>
          ...
          <vf-card-footer>
            {/* <vf-flex direction="column" halign="center" gap={4}> */}
            <vf-button class="verify-button" onSubmit={this.handleSubmit}>
              Verify
            </vf-button>
            <vf-flex direction="row" halign="center" gap={1}>
              <vf-label>Didn't receive the code?</vf-label>
              <vf-link href="/resend-code">Resend</vf-link>
            </vf-flex>
            {/* </vf-flex> */}
          </vf-card-footer>
        </div>
